...
final class ReportClientScript {
    ...
    private static final String SCRIPT_PART_2 =
            ...
            + "      // Persist the refreshed report via the shell, after all DOM mutations\n"
            + "      try {\n"
            + "        var saveBridge = window.parent && window.parent.__portfolioReportSaveBridge;\n"
            + "        if (saveBridge && typeof saveBridge.save === 'function') {\n"
            + "          var refreshedHtml = '<!DOCTYPE html>\\n' + document.documentElement.outerHTML;\n"
            + "          Promise.resolve(saveBridge.save(refreshedHtml)).catch(function(saveError) {\n"
            + "            if (status) status.textContent += ' (could not save refreshed report)';\n"
            + "          });\n"
            + "        }\n"
            + "      } catch (saveBridgeError) {}\n"
            + "    } catch (error) {\n"
